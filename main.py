...
def parse_html(html: str) -> list[str]:
    """
    Extracts all .pdf URLs from anchor tags in the given HTML content.
    ...
    """
    pattern = r'href="([^"]+\.pdf)"'
    pdf_urls = re.findall(pattern, html)
    return pdf_urls
...
def main() -> None:
    html_file_path: str = "balchem.com.html"  # Name of HTML file
    ...
    if not check_file_exists(
        ...
        url: str = f"https://www.cfindustries.com/products/safety-data-sheets"
        print(f"Visiting: {url}")
        save_html_with_selenium(url=url, output_file=html_file_path)  # Save HTML content to file
    ...
    if check_file_exists(system_path=html_file_path):  # Check if HTML file exists
        ...
        for pdf_link in pdf_links:  # For each PDF link
            if not validate_url(given_url=pdf_link):
                pdf_link = "https://www.cfindustries.com" + pdf_link
